...
public class PartnerMerchantController {
    ...
    /**
     * Delete one of MY sub-partners (and its own claims, campaigns, products
     * and login accounts). Only the partner that owns the sub-partner can do this.
     */
    @DeleteMapping("/sub-merchants/{subId}")
    public ResponseEntity<?> deleteSubMerchant(@PathVariable Long subId, HttpServletRequest request) {
        Long me = myMerchantId(request);
        if (me == null) {
            return ResponseEntity.status(403).body("No merchant linked to this account");
        }
        ...
        try {
            claimRepository.findByMerchantId(subId).forEach(c -> claimRepository.delete(c));
            campaignRepository.findByMerchantId(subId).forEach(c -> campaignRepository.delete(c));
            productRepository.findByMerchantId(subId).forEach(p -> productRepository.delete(p));
            userRepository.findAll().stream()
                    .filter(u -> subId.equals(u.getMerchantId()))
                    .forEach(u -> userRepository.delete(u));
            merchantRepository.deleteById(subId);
            return ResponseEntity.ok(Map.of("message", "Sub-partner deleted"));
        } catch (Exception e) {
